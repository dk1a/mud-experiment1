...
contract MovementSystem is System {
  ...
  function execute(bytes memory args) public returns (bytes memory) {
    ...
    if (!LibPosition.isOccupied(components, toPosition)) {
    ...
    // if occupied, attack!
      ...
      LibAttack.strikeAtPosition(components, entity, toPosition);
    }
  ...
  function executeTyped(
    ...
    return execute(abi.encode(arenaEntity, toPosition));
  }
}
